 ...
 */
public class PropertiesContextBuilder {
    ...
    private class PropertiesContextImpl implements PropertiesContext {
        ...
        public Path get(String typeName) {
            return Paths.get(this.getDirPath(), this.getFileName(typeName));
        }

        @Override
        public InputStream getInputStream(String typeName) throws IOException {
            final Path path = this.get(typeName);
            final InputStream output = new GetInputStreamsForPath(classLoader).getSingle(path);
            return output;
        }
        ...
        public List<InputStream> getInputStreams(String typeName) throws IOException {
            final Path path = this.get(typeName);
            final List<InputStream> output = new GetInputStreamsForPath(classLoader).get(path);
            return output;
        }
        ...
        public OutputStream getOutputStream(String typeName, boolean append) throws IOException {
            final Path path = this.get(typeName);
            final OutputStream output = new GetOutputStreamsForPath(classLoader, append).getSingle(path);
            return output;
        }
        ...
        public List<OutputStream> getOutputStreams(String typeName, boolean append) throws IOException {
            final Path path = this.get(typeName);
            final List<OutputStream> output = new GetOutputStreamsForPath(classLoader, append).get(path);
            return output;
        }
